...
export function parseUrl(url) {
  var tmp, res = {};
  res["href"] = url;
  for (const p in urlRegex) {
      tmp = urlRegex[p].exec(url);
      res[p] = tmp[1];
      url = tmp[2];
      if (url === "") {
          url = "/";
...
export function getParams(regexUrl, url) {
  ...
  const regex = pathToRegexp.pathToRegexp(regexUrl.replace(host, ''), keys)
  // 排除url参数部分
...
export function appendQuery(url, query) {
  query = typeof query === 'string' ? pathToRegexp.parse(query) : query
  const path = url.split('?')[0]
  const originalQuery = pathToRegexp.parse(url)
